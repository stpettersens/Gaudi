...
def configureBuild(args):
	...
	amendAntBld(5, "<!-- Partially generated by configure script. -->", system_family, True)
	amendSource(1, 'GaudiPluginSupport', '/\*', '/*', system_family, True)
	amendSource(1, 'GaudiApp', '/\*', '/*', system_family, True)
	if not use_groovy and not use_jython:
		amendSource(29, 'GaudiPluginSupport', '//', '/*', system_family, False)
		amendSource(31, 'GaudiPluginSupport', '//', '*/', system_family, False)
		amendSource(32, 'GaudiPluginSupport', '/\*', '//', system_family, False)
		amendSource(34, 'GaudiPluginSupport', '\*/', '//', system_family, False)
	...
	if use_gtk:
		...
		system_family, False)
		amendSource(24, 'GaudiApp', '//', 'import org.gnome.Gtk', system_family, False)
		amendSource(52, 'GaudiApp', '/\*', '//', system_family, False)
		amendSource(57, 'GaudiApp', '\*/', '//', system_family, False)
...
def amendAntBld(line_num, new_line, osys, create):
	"""
...
def amendSource(line_num, src_file, match, new_line, osys, create):
	prefix = 'src/org/stpettersens/gaudi/'
	if create:
		# Copy {src}.scala_ -> {src}.scala
		shutil.copyfile('{1}{0}.scala_'.format(src_file, prefix), '{1}{0}.scala'.format(src_file, prefix))
	src_file = src_file + '.scala'
	txtrevise = useScript('txtrevise')
	command = '{4} -q -f {5}{0} -l {1} -m "{2}" -r "{3}"'.format(src_file, line_num, match, new_line, txtrevise, prefix)
	execChange(command, osys)
...
def execChange(command, osys):
	global use_script
	if use_script and re.match('\*nix|darwin', osys):
		os.system('./' + command)
	else:
		os.system(command)
